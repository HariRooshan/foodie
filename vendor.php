...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $active_check = $conn->query("SELECT COUNT(*) AS active_users FROM vendors WHERE is_active = TRUE");
    ...
    if ($active_users >= 10) {
        ...
        $stmt = $conn->prepare("SELECT vendor_id, name, email, password_hash, is_active FROM vendors WHERE email = ?");
        ...
        if ($result->num_rows === 1) {
            ...
            if (password_verify($password, $user['password_hash'])) {
                ...
                // Mark user as active
                $update = $conn->prepare("UPDATE vendors SET is_active = TRUE WHERE vendor_id = ?");
                $update->bind_param("i", $user['vendor_id']);
                $update->execute();
                $update->close();
...
<body>
    ...
    <div class="login-container">
        <div class="login-card">
            <div class="brand-title">Hello, Vendor!</div>
